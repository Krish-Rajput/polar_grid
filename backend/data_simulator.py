...
class PolarDataSimulator:
    ...
    def generate_hourly_data(self, days: int = 365, start_date: datetime = None) -> pd.DataFrame:
        ...
        for hour in range(days * 24):
            ...
            # Antarctica: Polar night (May-Aug), midnight sun (Nov-Feb)
            if self.station_type == "antarctica":
                if 120 <= day_of_year <= 244:  # May 1 - Aug 31 (polar night)
                    solar = 0.0
                elif 304 <= day_of_year or day_of_year <= 31:  # Nov-Feb (midnight sun)
                ...
                if 320 <= day_of_year or day_of_year <= 40:  # Nov-Jan (polar night)
                    solar = 0.0
                elif 130 <= day_of_year <= 214:  # May-Aug (midnight sun)
            ...
            # === LOAD MODELS ===
            # Base load (life support, heating) - higher in winter
            heating_factor = max(0, (-temp - 10) / 30)  # More heating when colder
            base = self.base_load * (1 + 0.5 * heating_factor)
            base += np.random.normal(0, 5)
            
            # Research load - higher in summer (more activity)
            if self.station_type == "antarctica":
            ...
            # Water desalination - constant but can be shifted
            ...
            # Wind generation (cut-in at 3 m/s, rated at 12 m/s, cut-out at 25 m/s)
            if wind < 3:
                wind_gen = 0
            elif wind < 12:
                ...
                wind_gen = 0  # Cut-out for safety
            
            wind_gen *= np.random.uniform(0.9, 1.0)  # Efficiency losses
            ...
            if renewable >= demand:
                ...
                current_soc = min(95, current_soc)  # Max safe SoC
                gen_output = 0
                diesel = 0
            else:
                # Need generator + battery
                deficit = demand - renewable
                if current_soc > 20:  # Use battery
                    ...
                    gen_output = deficit
                    current_soc = 20  # Minimum safe SoC
                
                # Generator efficiency: ~2.5 liters/kWh
...
if __name__ == "__main__":
    ...
    print("=== Polar Station Energy Data Summary ===")
    ...
    print(f"Total demand range: {df['total_demand_kw'].min():.1f} to {df['total_demand_kw'].max():.1f} kW")
    print(f"Solar generation: {df['solar_generation_kw'].sum():.0f} kWh/year")
    print(f"Wind generation: {df['wind_generation_kw'].sum():.0f} kWh/year")
    print(f"Diesel consumption: {df['diesel_consumption_liters'].sum():.0f} liters/year")
    print(f"Diesel cost (80/liter): ₹{df['diesel_consumption_liters'].sum() * 80:,.0f}")
